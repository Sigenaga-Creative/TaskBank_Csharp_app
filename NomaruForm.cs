...
namespace SYUUKAN
{
    public partial class NomaruForm : Form
    {
        ...
        public NomaruForm(Home _home)
        {
            ...
            TaitoruHaikei.Image = Image.FromFile("ノーマル背景.png");
            Kakutei.Image = Image.FromFile("横確定ボタン.png");
            Hatena.Image = Image.FromFile("ハテナボタン.png");
            CommonFunction.MoziEvent(NormalTask, "追加するタスクを入力してください。");   //テキストボックスのイベント追加
            CommonFunction.SetMozi(NormalTask, "追加するタスクを入力してください。");    //テキストボックスのリセット

            this.Click += Form_ClickOut;  // ← フォーム全体にClickイベント追加

            //パネルの設定
            NomaruTaskRan.FlowDirection = FlowDirection.TopDown;        //パネルを縦方向にそろえる
            NomaruTaskRan.WrapContents = false;
            NomaruTaskRan.AutoScroll = true;                     // はみ出したらスクロール
            NomaruTaskRan.Padding = new Padding(2);              // 内側余白
        ...
        private void NomaruForm_Load(object sender, EventArgs e)
        {
            LoadTask();
        ...
        private void AddTaskToForm(int id, string name, bool completed,int TaskCount)
        {
            ...
            panel.Height = 35;
            panel.Width = NomaruTaskRan.ClientSize.Width - 30;
            ...
            lblId.Margin = new Padding(0, 1, 0, 1);
            lblId.BackColor = Color.FromArgb(150, 180, 248);
            ...
            txtName.Margin = new Padding(0, 1, 0, 1);
            ...
            txtName.Leave += (s, e) =>      //処理を離れると名前をデータベースに保存し、読み込み専用へ
            {
                ...
                NormalUpdateName(id, txtName.Text);
            };
            ...
            btnComplete.Margin = new Padding(0, 1, 0, 1);
            ...
            btnDelete.Margin = new Padding(0, 1, 0, 1);
            ...
            // パネルの表示
            NomaruTaskRan.Controls.Add(panel);
        ...
        {
            NomaruTaskRan.Controls.Clear();     // ← 表示エリアをリセット(古いものの重複防止)
            using (SqlConnection con = new SqlConnection(connectionDB))
            {
                con.Open();
                string query = "SELECT * FROM NormalTasks ORDER BY Id";
        ...
        private void NameInsert(string taskName)
        {
            ...
            {
                conn.Open();
                string query = "Insert INTO NormalTasks (TaskName) VALUES (@name)";
        ...
        private void IdDelete(int id)
        {
            ...
            {
                con.Open();
                string query = "DELETE FROM NormalTasks WHERE Id = @id";
                SqlCommand cmd = new SqlCommand(query, con);
        ...
        private int GetTaskCount()
        {
            ...
            {
                conn.Open();
                string q = "SELECT COUNT(*) FROM NormalTasks";
        ...
        //タスクの名前更新
        private void NormalUpdateName(int id, string newName)
        {
            using (SqlConnection con = new SqlConnection(connectionDB))
            {
                con.Open();
                string query = "UPDATE NormalTasks SET TaskName = @name WHERE Id = @id";
                SqlCommand cmd = new SqlCommand(query, con);
        ...
        private string GetName(int id)
        {
            using (SqlConnection con = new SqlConnection(connectionDB))
            {
                con.Open();
                // 名前の取得
                string query = "SELECT TaskName FROM NormalTasks WHERE Id = @id";
                string name;
        ...
        {
            //ノーマルの報酬額を取得
            int reward = CommonDB.ALLRewardGet.GetRewardValue("NormalReward");
            ...
            // タスクの達成状態も更新(ノーマルはなくても良い処理)
            TrueTassei(taskId);

            string name = GetName(taskId);
            // 達成完了メッセージ
            MessageBox.Show($"タスクを達成しました！",
                            "【ノーマル】", MessageBoxButtons.OK, MessageBoxIcon.Information);
            // ホーム画面のセリフを変更
        ...
        private void NomaruForm_Shown(object sender, EventArgs e)
        {
            this.ActiveControl = null;      //テキストボックスに選択されるのを防ぐ
        }
        ...
        {
            home.KetteiOto();
            string taskName = NormalTask.Text.Trim();

            // 入力チェック
            if (string.IsNullOrWhiteSpace(taskName) || taskName == "追加するタスクを入力してください。")
            {
                CommonFunction.SetMozi(NormalTask, "追加するタスクを入力してください。");
        ...
        //テキストボックスのリセット
        private void NormalTask_Enter(object sender, EventArgs e)
        {
            home.KetteiOto();
            CommonFunction.SetMozi(NormalTask, "追加するタスクを入力してください。");    //テキストボックスのリセット
        }
        ...
        private void NormalTask_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        {
            home.KetteiOto();
            CommonFunction.MoziKesu(NormalTask);
        }
        ...
        private void Hatena_Click(object sender, EventArgs e)
        {
            ...
            CommonFunction.ShowInfo(
                "【ノーマル】",
                "1日に何度でもこなせて、一番シンプルなモードです！\n" +
                "“やった方がいいけど後回しにしがちなこと”や、“毎日続けたいこと”などを登録しておくのがおすすめです。\n\n" +
                "・「達成」ボタンでお小遣いGET！\n" +
                "※　報酬金額は設定画面から変更できます。\n\n" +
                "・「削除」ボタンでタスクを削除\n" +
